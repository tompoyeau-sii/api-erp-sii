            ...
            .then((customers) => {
                ...
                caGlobalCustomers.forEach((month) => {
                    if (today(year) >= month.start_month && today(year) <= month.end_month) {
                        caOfActualMonthCustomer.push({ id: month.id, label: month.label, value: month.value, total_pru: month.total_pru, total_tjm: month.total_tjm, Projects: month.Projects });
                    }
                });
